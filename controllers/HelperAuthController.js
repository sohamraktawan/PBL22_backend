...
module.exports.signup_post = async (req, res) => {
    ...
    try{
        ...
        let requestsPending = [];
        const user = await Helper.create({username, email, password, requestsPending});
...
module.exports.login_post = async (req, res) =>{
    ...
    try{
        const user = await Helper.login(email, password);
        const token = createToken(user._id);
        res.send(token);
...
module.exports.auth = async (req,res) =>{
    ...
    jwt.verify(token, 'soham post application secret', async (err, decodedToken)=>{
        if(err){
            ...
            let user =  await Helper.findById(decodedToken.id)
            //console.log(user)
            auth = "true"
            res.send(user)
        }
